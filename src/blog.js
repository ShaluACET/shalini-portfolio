import './App.css';
function Blog() {
  return (
    <div>
      <div className="background">
      <nav className="navbar">
        <div className="logo">My Blog</div>
        <div className="menu">
          <button className="menu-item">Home</button>
          <button className="menu-item">About</button>
          <button className="menu-item">Services</button>
          <button className="menu-item">Contact</button>
        </div>
      </nav>
      <div>
      <nav className="header">
        <h5>Hey, I'M</h5>
        <h1>R SHALINI</h1>
        <p className="styled-paragraph">A frontend focused Web Developer buliding the frontend of Websites & Web<br/>Appliations that leads to the success of the overall projects</p>
      </nav>
      </div>
    </div>
    </div>
  );
}
export default Blog;
